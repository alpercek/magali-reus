<template>
  <div class="slider-container">
    ...
    >
      <div
        v-for="(slide, index) in publication.data.slides"
        :id="`s-${index}`"
        :key="index"
        class="slide"
      >
        <img class="slide-image" :src="slide.slide.url" />
      </div>
    </div>
...
export default {
  ...
  computed: {
    activeSlide () {
      return this.publication.data.slides[this.active]
    }
  },
  methods: {
    slideTo (index) {
    ...
    navigate () {
      ...
      } else if (this.active < this.publication.data.slides.length - 1 && this.direction === 1) {
        this.slideTo(this.active + this.direction)
      }
    },
    changeCursor (e) {
